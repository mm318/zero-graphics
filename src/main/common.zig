...
export fn zerog_renderer2d_alloc(user_data: ?*anyopaque, size: usize) ?*anyopaque {
    ...
    const allocator = @as(*std.mem.Allocator, @ptrCast(@alignCast(@alignOf(*std.mem.Allocator), user_data orelse @panic("unexpected NULl!"))));
...
export fn zerog_memset(ptr: ?[*]u8, value: c_int, num: usize) ?[*]u8 {
    ...
    std.mem.set(u8, ptr.?[0..num], @as(u8, @intCast(value)));
    return ptr;
}
